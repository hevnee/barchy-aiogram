...
@router.message(Command("start"), is_not_forwarded)
async def start_command(message: types.Message, state: FSMContext):
    ...
    if lang is None:
        ...
        await state.update_data(command=partial(start_command, message, state))
        return await message.reply(text=SELECTED_LANGUAGE["none"], reply_markup=language_keyboard)

    await message.reply(SELECTED_LANGUAGE[lang]["welcome_text"])